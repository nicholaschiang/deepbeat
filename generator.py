...
GPU run command:
    ...
    Note: running Keras/Theano on GPU is formally supported for only NVIDIA
    cards (CUDA backend).
"""
...
def __sample(a, temperature=1.0):
    a = np.array(a, dtype="float64")  # We get sum() errors with dtype="float32"
